 ...
 * @lastEditors: wanglu
 * @Date: 2019-08-16 14:12:53
 * @LastEditTime: 2019-09-12 18:17:38
...
class Editprovider extends RootComponent<FormProps,State>{
  ...
  providerInfo = (id: any) => {//获取供应商详情
    this.axios.request(this.api.storeSupplierInfo,{ id }).then(({ data }) => {
      this.setState({
        information: data
      }, () => {
  ...
  updateInfo(params: any) {
    const { id } = HttpUtil.parseUrl(window.location.href)
    this.axios.request(this.api.storeEditSupplier, {...params, id}).then(({ code })=> {
...
export default Form.create<FormProps>()(Editprovider)
